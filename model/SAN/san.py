from model.SAN import common
import torch
import torch.nn as nn
import torch.nn.functional as F
from model.SAN.MPNCOV.python import MPNCOV
...
class SAN(nn.Module):
    def __init__(self, n_g, n_b, n_f, reduction, scale, conv=common.default_conv):
        ...
        # define head module
        modules_head = [conv(3, n_f, kernel_size)]
        ...
        self.gamma = nn.Parameter(torch.zeros(1))
        # self.gamma = 0.2
        self.n_resgroups = n_g
        self.RG = nn.ModuleList([LSRAG(conv, n_f, kernel_size, reduction, \
                                              act=act, res_scale=1, n_resblocks=n_b) for _ in range(n_g)])
        self.conv_last = conv(n_f, n_f, kernel_size)
        ...
        # define tail module
        modules_tail = [
            common.Upsampler(conv, scale, n_f, act=False),
            conv(n_f, 3, kernel_size)]

        self.add_mean = common.MeanShift(255, rgb_mean, rgb_std, 1)
        self.non_local = Nonlocal_CA(in_feat=n_f, inter_feat=n_f//8, reduction=8,sub_sample=False, bn_layer=False)
