...
class ABPN_v7(nn.Module):
    def __init__(self, input_dim, dim):
        super(ABPN_v7, self).__init__()
        kernel_size = 6
        pad = 1
        stride = 4
        ...
        self.SA10 = Time_attention(dim, dim, 1, 1, 0, 1)
        # reconstruction
        self.SR_conv1 = ConvBlock(5 * dim, dim, 1, 1, 0)
        self.SR_conv2 = ConvBlock(dim, dim, 3, 1, 1)
        self.LR_conv1 = ConvBlock(4 * dim, dim, 1, 1, 0)
        self.LR_conv2 = UpBlock(dim, dim, kernel_size, stride, pad)
        self.SR_conv3 = nn.Conv2d(dim, input_dim, 3, 1, 1)
        # BP final
        self.final_feat1 = ConvBlock(input_dim, 2 * dim, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        down4 = self.SA4(self.weight_down2(down2), down4)
        # BP 10
        up10 = self.up10(down4) + self.weight_up8(up3)
        # reconstruction
        HR_feat = torch.cat((up1, up2, up3, up4, up10), 1)
        LR_feat = torch.cat((down1, down2, down3, down4), 1)
        HR_feat = self.SR_conv1(HR_feat)
        HR_feat = self.SR_conv2(HR_feat)
        LR_feat = self.LR_conv1(LR_feat)
        LR_feat = self.LR_conv2(LR_feat)
...
class ABPN_v8(nn.Module):
    def __init__(self, input_dim, dim):
        super(ABPN_v8, self).__init__()
        kernel_size = 6
        pad = 2
        stride = 2
    ...
    def forward(self, x):
        ...
        LR_res = x - F.interpolate(SR, scale_factor=0.5, mode='bicubic')
        LR_res = self.final_feat1(LR_res)
        LR_SA = self.final_SA0(LR_res)
        LR_res = self.final_feat2(LR_SA)

        SR_res = F.interpolate(LR_res, scale_factor=2, mode='bicubic')
