...
class Trainer:
    ...
    def train_one_epoch(self):
        ...
        train_dataloader = DataLoader(
            dataset=self.dataset, num_workers=self.opt.num_thread, batch_size=self.opt.num_batch, pin_memory=True, shuffle=True
        )
        loss_total = []
    ...
    def validate_frame_all(self):
        ...
        with torch.no_grad():
            ...
            for idx in range(len(self.dataset.lr_images)):
                ...
                target_tensor = torch.squeeze(torch.clamp(target_tensor, min=0, max=1.0), 0).permute(1, 2, 0)
                target_tensor *= 255
